---
title: ICPC Xi'an Invitational 2026 Review
description: ICPC Xi'an Invitational 2026 Review
...
> There were almost no operations-side issues in this invitational (I really cannot do anything about machines that are too underpowered and crash). Only parts of the workflow were adjusted slightly. This article may be expanded later (depending on how many details I can remember).

## Script Optimizations

### Excel Random Password Generation

Fixed the randomness issue and ensured there are no duplicates.

{{% details summary="VBA Code" %}}
...
Function RandomPassword(length As Integer) As String
    ...
    If length < 1 Or length > 128 Then
        RandomPassword = "ERROR: Password length must be between 1 and 128"
        Exit Function
    End If
    ...
    attempts = 0
    On Error Resume Next          ' Set only once
    ...
    Do
        ...
        If attempts > 10000 Then
            ...
            Err.Raise vbObjectError + 513, "RandomPassword", _
                "Failed to generate a unique password within 10000 attempts. Please check the length parameter."
        End If
    Loop
...
Sub ResetPasswordGenerator()
    ...
    IsRandomized = False
    MsgBox "Password generator has been reset.", vbInformation
End Sub
``` 

{{% /details %}}
### Printing Script (DomJudge)

{{% details summary="Reference Code" %}}
...
### Printing Script (Client)

A `SumatraPDF.exe` needs to be downloaded and placed in the corresponding location. This prevents the print job from getting stuck in the queue when the printer does not support directly printing a raw PDF stream.
...
Before the official contest, I used the newly written [Onyx stress-testing tool](https://github.com/4o3F/Onyx) to stress-test the full workflow. It worked well and could uncover problems in advance very effectively.  
DOMjudge's official stress-testing tool has not been updated for years, and it is written in Scala, which is honestly hard to take.

### Scoreboard Reveal

For this scoreboard reveal, the newly written [Pyrite scoreboard reveal tool](https://github.com/4o3F/Pyrite) was tested in production for the first time. So far it looks acceptable; detailed usage instructions are available in the README on GitHub.  
The remaining issues are currently as follows:

+ The host cannot click once and directly move to the reveal of the next award, so someone offstage has to coordinate and click along, which can lead to coordination mistakes. This should later be optimized so one click jumps directly to the next award reveal.
+ Add a configuration option so that after `n` teams finish their scoreboard reveal, those `n` teams are shown in a scrolling display before entering the next award group.

### Contestant Machine Related

Because we fully switched to the new 2026 image this time, Natsume and the accompanying client configure scripts received quite a few fixes. Local TLS termination was also added to support Brotli compression.  
For details, see the subsequently updated Natsume README, or check the recent commit log.

## Contest Live Deploy - Complete Build Process

Build a Debian Live USB deployment disk from a raw disk image. It supports both empty-disk and dual-system (Windows+Linux) scenarios, with manual confirmation required at key steps.

### Prerequisites

- A Debian/Ubuntu machine (for building)
- The original raw disk image file (assumed path: `/path/to/contest.img`)
...
### Step 1: Extract rootfs from the raw image and pack it as squashfs

{{% details summary="Extract rootfs and pack squashfs" %}}
...
# Mount the raw image
# kpartx automatically parses the partition table and creates /dev/mapper/loopXpN devices
sudo kpartx -av /path/to/contest.img

# View partition mappings and find the Linux root partition (usually the largest ext4 partition)
lsblk -f /dev/loop0    # Confirm which partition is rootfs

# Mount the rootfs partition (loop0p4 is the ext4 rootfs)
sudo mkdir -p /mnt/img-root
sudo mount /dev/mapper/loop0p4 /mnt/img-root

# Confirm the mounted contents are correct
ls /mnt/img-root/
# You should see standard Linux directories such as bin/ etc/ home/ usr/ var/

# Pack as squashfs (exclude unnecessary directories)
...
> **Note**: If the rootfs in the image is not `loop0p4`, adjust it according to the output of `lsblk -f /dev/loop0`.
> If compression is too slow or decompression is slow on the target machines, you can switch to `-comp lz4` to trade compression ratio for speed.

### Step 2: Initialize the live-build project

{{% details summary="Initialize the live-build project" %}}

```bash
mkdir -p ~/contest-live && cd ~/contest-live

# If starting from scratch, clean old state (keep cache)
...
Because manual interactive confirmation is required, it is not suitable to put this in a live-config hook (hooks run in the background and have no TTY).
Instead: **put the script in PATH, and automatically start it on login through autologin + `.bash_profile`.**

#### 5a: Installation script

{{% details summary="contest-install installation script" %}}
...
# ════════════════════════════════════════════════
# PHASE 2: Analyze partitions — determine empty disk or dual-boot
# ════════════════════════════════════════════════
...
if [[ "$PART_COUNT" -eq 0 ]]; then
    ...
    while IFS= read -r part; do
        ...
        # Microsoft partition (by GUID) — but check actual filesystem first
        case "${part_type^^}" in
        ...
        # Linux partition (by standard Linux GUID) — use the first one as the target
    ...
    # Determine mode: EFI + Linux partition present → dual-boot (preserve other partitions)
...
# ════════════════════════════════════════════════
# PHASE 3: Choose partition strategy + manual confirmation
# ════════════════════════════════════════════════
...
#### 5b: Autologin and script startup

Debian Live's `live-config` creates the default user `user` and logs in automatically, so:
- Put `.bash_profile` under `/etc/skel/` (it will be copied when `user` is created)
- Configure passwordless sudo, and invoke the script through `sudo`

```bash
# Default user's .bash_profile — automatically run the installation script on login
...
Configure passwordless sudo (allow the default user to execute the installation script without a password):
...
### Step 6: Configure the GRUB boot menu
...
> Build time depends on network speed and squashfs size, usually 10-30 minutes.
> Output file: `live-image-amd64.hybrid.iso`

### Step 8: Write to USB drive

```bash
# Confirm the USB drive device (replace sdX according to your actual situation)
lsblk

# Write the image (iso-hybrid format can be written directly with dd)
sudo dd if=live-image-amd64.hybrid.iso of=/dev/sdX bs=4M status=progress oflag=sync
```

> For batch deployment, prepare multiple USB drives and `dd` one copy to each.
...
├── config/
│   ├── package-lists/
│   │   └── contest.list.chroot                           # Additional packages to install
│   ├── includes.chroot/
│   │   ├── usr/local/bin/
│   │   │   └── contest-install                           # Installation script (interactive)
│   │   ├── usr/local/share/icpc/
│   │   │   └── filesystem.squashfs                       # Target system rootfs
│   │   ├── etc/skel/
│   │   │   └── .bash_profile                             # Auto-start installation script
│   │   └── etc/sudoers.d/
│   │       └── contest-install                           # Passwordless sudo
│   └── includes.binary/
│       └── boot/grub/
│           └── grub.cfg                                  # GRUB boot menu
└── live-image-amd64.hybrid.iso                           # Final artifact
```

### Deployment workflow (onsite operation)

```
1. Insert the USB drive into the target machine → choose USB boot in UEFI
2. GRUB menu 5-second countdown → "Contest Installer"
3. live-config autologin as user → 10s auto-start installer
4. Script interaction flow:
   ┌─────────────────────────────────────────────────┐
   │ Show all disks → [manual] enter target disk name           │
   │ Analyze partitions → automatically determine: empty disk or dual-boot            │
   │   Empty disk: create GPT + EFI + root                │
   │   Dual-boot: preserve Windows, overwrite only the Linux partition      │
   │ → [manual] confirm partition operation                           │
   │ Execute partitioning + formatting                               │
   │ → [manual] confirm continuing installation                           │
   │ Extract squashfs + install GRUB                       │
   │ → [manual] press Enter to reboot                             │
   └─────────────────────────────────────────────────┘
5. Remove the USB drive and boot from the hard disk
```

About 5-10 minutes per machine, including manual confirmation time.
...
{{% details summary="JudgeHost configuration script" %}}
